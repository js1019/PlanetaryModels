...
clear all; clc;
load('../mars/marsDWAK_3L_gravity.mat');
global RI epsl;
...
%plot(RI0,eta);
epa0 = 5/2*Re*1e3/g0/(eta(1)+2)*(2*pi/(Tp*3600))^2;
% change !! epa to the observed value
epa = 589e-5;

epsl = zeros(lz,1); epsl(:) = epa0; 
...
% edit the solid regions
ed = linspace(epa/epa0,1.0,lz-RD(2,2));
epsl(1:lz-RD(2,2)) = epsl(1:lz-RD(2,2)).*ed';
...
rmax = Re*(1+epa/3); nr = 1500; ny = 30;
xr = linspace(0,rmax,nr); co2 = linspace(0,1,ny);
tic
axr = zeros(nr,ny); a0 = 0; a1 = rmax;
for i = 2:nr
   for j = 1:ny
...
toc
%imagesc(axr)
clear d0 d1 i j lz RI0 a0 a1 ed