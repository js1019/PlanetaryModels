...
disp('')
'Stokes particle target FMM in R^3'

tic
iprec=1
[U]=stfmm3dpart(iprec,nsource,source,ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,ifpot,ifgrad,ntarget,target,ifpottarg,ifgradtarg);
total_time=toc

'Stokes particle direct evaluation in R^3'

tic
[F]=st3dpartdirect(nsource,source,ifsingle,sigma_sl,ifdouble,sigma_dl,sigma_dv,ifpot,ifgrad,ntarget,target,ifpottarg,ifgradtarg);
total_time=toc
